...
export const EV_BOOST_ITEMS171=Object.entries(STAT_LABELS122).map(([stat,label])=>({name:label+'アップの実',kind:'evBoost',stat,amount:10,price:3000,desc:label+'の努力値を10上げる。能力ごと252、合計510まで。'}));
export const EV_ITEMS122=[...EV_BOOST_ITEMS171,...Object.entries(STAT_LABELS122).map(([stat,label])=>({name:label+'リセットの実',kind:'evReduce',stat,amount:10,price:500,desc:label+'の努力値を10減らす。0より下にはならない。'}))];
