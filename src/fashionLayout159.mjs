...
export function upgradeFashion159(base,d){
 ...
 const actors=d.actors.map((a,i)=>a.id==='n:0'?{...a,x:5,y:3,dir:'down',mode:'still'}:a.type==='person'?{...a,template:String([19,20,22][(i-1)%3])}:a);
 for(const [i,x,y,v]of [[0,3,9,18],[1,14,5,20]]){if(actors.some(a=>!a.stored79&&a.x===x&&a.y===y))continue;const id='a:fashion159-customer'+i;actors.push({id,type:'person',template:String(v),name:customerNames159[base.id+':'+id]||freshName138(base.id+':'+id,actors.map(a=>a.name).filter(Boolean)),x,y,dir:i?'left':'up',mode:'still',owner:null,talk:i?'どの色の服にしようかな。':'ガオンとのお出かけに合う服を探しているんだ。'});}
