...
export function chooseAppearance() {
  return showForm({title:'きみの みためは？',sub:'すきな みためを えらんでね。',fields:[],submit:'この すがたで はじめる',mount(host){
    const look={gender:'boy',appearanceVersion:1,hair:'',shirt:'',pants:'',hairMap119:'default',outfit119:'default'};
    ...
    const styles=document.createElement('link');styles.rel='stylesheet';styles.href=new URL('./gate127.css',import.meta.url).href;host.append(styles);
    ...
    function group(key,title,choices){
      ...
      field.dataset.part=key;field.append(legend,buttons,caption);host.append(field);
      const entries=choices.map(choice=>{const button=document.createElement('button');button.type='button';button.dataset.value=choice.value;button.setAttribute('aria-label',choice.name);button.title=choice.name;
        if(choice.color){button.className='swatch';button.style.background=choice.color;}else{button.textContent=choice.name;button.style.flex='1 1 140px';}
    ...
    group('outfit119','ふくの いろ',OUTFITS119.map(([value,name])=>({value,name})));
    group('hair','かみの いろ（50色）',[{name:'もとの髪色',value:''},...APPEARANCE_COLORS.map(c=>({...c,value:c.color}))]);
