...
export async function chapterNpc(world,n){
 ...
 if(n.script==='v5:professor'){
  ...
  if(!flag('v5:netGift')){
   await ui.say(['山おくで ラテットに会ったのか！','すぐに 逃げてしまったのじゃな。']);
   await ui.say(['ガオンをもっと調べてみないか？']);
   await ui.say(['ガオンを調べるためには つかまえて そだててみないと わからないもんじゃ。']);
   await ui.say(['ガオンをつかまえるために ひつような どうぐを あげよう。']);
   await ui.say(['あまり かずは あげられないが 大切につかうんじゃぞ。']);
   addItem('ラグネット',15);setFlag('v5:netGift');setFlag('gotNet');beep('levelup');await persist();
   await ui.say([State.save.name+'は ラグネットを１５こ もらった！']);
   await ui.say(['これで 野生のガオンを つかまえられる。','まずは 1ばんどうろを抜けて','となりの ロッズタウンへ 行ってごらん。']);return;
  }
