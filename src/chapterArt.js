...
import {drawMap173} from './terrainArt173.js';
...
import {drawResourceTree85,resourceArtReady85} from './resource80.js';
import {drawWaterfalls84} from './waterfall84.js';
import {drawReadable84,readableReady84} from './readableArt84.js';
import {drawWater83} from './waterSurface83.js';
import {drawTree83,drawActive83,activeReady83} from './activeArt83.js';
import {isTree83,treeVisual83} from './treeFootprint83.mjs';
...
function drawChapterBase72(ctx,map,camX,camY){
 ...
 if((!map.interior123||interiorReady123())&&alpineReady65(map)&&roadReady(map)&&environmentReady(map)&&(!map.forestBorder||forestReady())&&(map.id!=='mountain'||mountainReady())&&grassReady(map)&&(!(map.biome||map.townPond)||marineReady()))furnitureReady128()&&shopsReady74()&&editorGroundReady72(map)&&cache.set(map,cv);}
...
export function drawEditorProp72(c,p,map){if(p.color115)return tintMaterial115(c,p.color115,(p.x-1)*32,(p.y-2)*32,(p.w+2)*32,(p.h+4)*32,l=>drawEditorProp72(l,{...p,color115:undefined},map));if(!isTree83(p)&&!p.turn81&&drawResourceTree85(c,map,p))return true;if(isTree83(p)&&!p.treeDraw83&&!p.turn81){return drawTree83(c,p,map);}if(p.turn81){c.save();c.translate((p.x+p.w/2)*32,(p.y+p.h/2)*32);c.rotate(p.turn81*Math.PI/2);drawEditorProp72(c,{...p,x:-p.originalW81/2,y:-p.originalH81/2,w:p.originalW81,h:p.originalH81,turn81:0},map);c.restore();return true;}if(/waterfall/i.test(p.art||''))return drawWaterfall173(c,p);if(drawLabProp177(c,p))return true;if(drawCliff173(c,p.art||p.key,p.x*32,p.y*32))return true;if(p.forest131){mountainMaterial(c,'fir',p.x*32,p.y*32,p.w*32,p.h*32);return true;}if(drawActive83(c,p))return true;if(drawReadable84(c,p.art,p.x*32,p.y*32,p.w*32,p.h*32))return true;if(p.art==='legacy73-chaletClinic')return drawMaterial(c,'chaletClinic',p.x*32,p.y*32,p.w*32,p.h*32);if(p.art==='wall79'){c.drawImage(tileFor('X',0,null,255,0,0,0,0),p.x*32,p.y*32,p.w*32,p.h*32);return true;}if(drawShop74(c,p))return true;if(p.art==='mountainCrag'){mountainMaterial(c,'crag',p.x*32,p.y*32,p.w*32,p.h*32);return true;}if(p.art==='shopCounter'){drawFurniture72(c,{theme:'home'},[['counter',p.x,p.y,p.w,p.h]]);return true;}if(drawNature73(c,p.art,p.x*32,p.y*32,p.w*32,p.h*32))return true;if(drawGarden72(c,p))return true;if(frontierAsset72(c,p.art,p.x*32,p.y*32,p.w*32,p.h*32))return true;if(environmentProp(c,p,map))return true;if(drawAlpineProp65(c,p,{...map,alpineRoute65:true},drawMaterial))return true;if(drawMarineAsset(c,p.art,p.x*32,p.y*32,p.w*32,p.h*32))return true;return drawMaterial(c,p.art,p.x*32,p.y*32,p.w*32,p.h*32);}
const editorViews73=new WeakMap();
export function drawChapterMap(c,map,camX,camY){if(drawMap173(c,map,camX,camY,drawEditorProp72,(ctx,t)=>drawEditorGround72(ctx,{editorGround72:[t]},drawMaterial))){c.save();c.translate(-camX,-camY);if(map.room?.furniture?.length)drawFurniture72(c,map.room,map.room.furniture);if(map.editorAddedFurniture72?.length)drawFurniture72(c,{theme:'home'},map.editorAddedFurniture72);for(const f of map.editorStyledFurniture73||[])drawFurniture72(c,f.room,[f.f]);for(const f of map.tabletopFurniture128||[])drawFurniture72(c,f.room,[f.f]);c.restore();drawWaterfalls84(c,map,camX,camY);return true;}let view=map;if(map.editorVisualRows73){view=editorViews73.get(map);if(!view){view={...map,rows:map.editorVisualRows73,editorDepth98:true};editorViews73.set(map,view);}}const result=drawChapterBase72(c,exitRoadView137(view),camX,camY);c.save();c.translate(-camX,-camY);for(const p of (map.editorVisualRows73?sortedProps98(map):map.editorAddedProps72||[]))drawEditorProp72(c,p,map);if(map.editorAddedFurniture72?.length)drawFurniture72(c,{theme:'home'},map.editorAddedFurniture72);for(const f of map.editorStyledFurniture73||[])drawFurniture72(c,f.room,[f.f]);for(const f of map.tabletopFurniture128||[])drawFurniture72(c,f.room,[f.f]);c.restore();drawWater83(c,map,camX,camY);drawPortShore163(c,map,camX,camY);drawWaterfalls84(c,map,camX,camY);drawShore173(c,map,camX,camY);return result;}
