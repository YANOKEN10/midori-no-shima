...
export function drawChapterMap(ctx,map,camX,camY){
 ...
 if(roadReady(map)&&environmentReady(map)&&(!map.forestBorder||forestReady())&&(map.id!=='mountain'||mountainReady())&&grassReady(map)&&(!map.biome||marineReady()))cache.set(map,cv);}
