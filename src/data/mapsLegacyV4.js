...
============================================================ */
  hut: {
    ...
    npcs: [
      {
        x: 7, y: 6, dir: "left", look: "girl", name: "母 ミレナ", script: "mother", noRoam: true,
...
============================================================ */
  mount1: {
    ...
    ledges: true,
    items: [{ x: 13, y: 12, item: "ガオンのくすり", flag: "m1heal" }],
    npcs: [
      {
...
const ROUTE_DEFS = [
  ...
  { id: "route2", name: "さばくの みち", south: "sand", southXY: [7, 1], north: "forest", northXY: [7, 12],
    ...
    list: [["スナムシ", 14, 17, 16], ["ツチマル", 14, 17, 14], ["イワッコ", 14, 17, 14],
           ["スナボンネ", 15, 18, 10], ["クルミグル", 14, 17, 12], ["アリンコ", 14, 17, 12],
           ["ヒバナリ", 15, 18, 10], ["モグポン", 15, 18, 12], ["ツチノコ", 14, 17, 12], ["スナコロネ", 15, 18, 10],
  ...
  { id: "route3", name: "モミの 回廊", south: "forest", southXY: [7, 1], north: "stone", northXY: [7, 12],
    ...
    list: [["キノコン", 20, 23, 14], ["ムシリン", 20, 23, 12], ["チョウマユ", 20, 23, 12],
           ["リボネム", 21, 24, 10], ["クモッコ", 20, 23, 12], ["ホタリン", 21, 24, 10],
           ["ウリボン", 21, 24, 12], ["タネコロ", 20, 23, 10], ["ヨルネコ", 21, 24, 8], ["リーフィン", 20, 23, 8], ["ハナビィ", 20, 23, 10],
           ["ネムノハ", 22, 25, 6], ["フクモッチ", 22, 25, 6], ["オバケシ", 21, 24, 8]],
    trainers: [
  ...
  { id: "route4", name: "いしきりの みち", south: "stone", southXY: [7, 1], north: "aqua", northXY: [7, 12],
    ...
    list: [["イワッコ", 26, 29, 14], ["ガンセキ", 27, 30, 10], ["コケゴロ", 26, 29, 12],
           ["クルミグル", 26, 29, 12], ["カセキン", 27, 30, 8], ["マグマゴ", 27, 30, 10],
           ["クロネズ", 26, 29, 12], ["カゲポン", 26, 29, 12], ["プラグン", 27, 30, 10], ["ドクロン", 27, 30, 8], ["スミビン", 28, 31, 6],
           ["デンデマリ", 28, 31, 6], ["ジシンヌシ", 29, 32, 4]],
    trainers: [
  ...
  { id: "route5", name: "水鏡の 桟道", south: "aqua", southXY: [7, 1], north: "sky", northXY: [7, 12],
    ...
    list: [["クラゲミ", 31, 34, 12], ["シズクン", 31, 34, 12], ["サカナビ", 31, 34, 12],
           ...
           ["ハネデン", 31, 34, 12], ["ヒツジン", 31, 34, 12], ["ミナモリス", 33, 36, 8], ["フワクジ", 32, 35, 8], ["ヌシガエル", 34, 37, 4],
           ["ウズシオヌシ", 34, 37, 4]],
    trainers: [
  ...
  { id: "route7", name: "星環鉄道沿い", south: "flame", southXY: [7, 1], north: "galaxy", northXY: [8, 14],
    ...
    list: [["メラボヤ", 41, 44, 10], ["ホノワン", 41, 44, 10], ["マグマゴ", 41, 44, 10],
           ...
           ["ハナヤリ", 42, 45, 8], ["デカネズ", 41, 44, 10], ["ホシミノ", 42, 45, 8], ["ヤミノヌシ", 44, 47, 3],
...
LEGACY_MAPS.inlet = {
  ...
  items: [{ x: 12, y: 8, item: "スーパーラグ", flag: "inletBall" }],
  npcs: [{ x: 6, y: 5, dir: "down", look: "sailor", name: "はまべの ひと",
           hair: "cap",
           talk: ["ここは まちの うらの いりえ。","しおかぜが きもちいいだろ？"] }],
  enc: { rate: 15, list: [["サカナビ", 10, 14, 14], ["カニポン", 11, 14, 14], ["シズクン", 12, 14, 12], ["アワミィ", 10, 14, 12], ["クラゲミ", 11, 14, 10], ["シオマント", 12, 14, 8]] },
};
...
LEGACY_MAPS.desert = {
  ...
  enc: { rate: 18, list: [["スナムシ", 16, 20, 14], ["ツチマル", 17, 20, 14], ["スナボンネ", 18, 20, 12], ["クルミグル", 16, 20, 12], ["モグポン", 17, 20, 10], ["ヒバナリ", 18, 20, 8]] },
};

LEGACY_MAPS.deepforest = {
  name: "樹海の 薬草道",
  kind: "out",
  rows: [
    "TTTTTTTTTTTTTTTT",
  ...
  enc: { rate: 20, list: [["キノコン", 22, 26, 14], ["チョウマユ", 23, 26, 14], ["ホタリン", 24, 26, 12], ["ネムノハ", 22, 26, 12], ["フクモッチ", 23, 26, 10], ["オバケシ", 24, 26, 8]] },
};
...
LEGACY_MAPS.cavern = {
  ...
  items: [{ x: 13, y: 11, item: "ハイパーラグ", flag: "cavernBall" }],
...
LEGACY_MAPS.river = {
  ...
  enc: { rate: 16, list: [["シズクン", 33, 37, 14], ["サカナビ", 34, 37, 14], ["ミナモン", 35, 37, 12], ["コオリン", 33, 37, 12], ["ラゲドン", 34, 37, 10], ["ミナモリス", 35, 37, 8]] },
};
...
LEGACY_MAPS.cloud = {
  ...
  items: [{ x: 12, y: 2, item: "ハイパーラグ", flag: "cloudBall" }],
...
LEGACY_MAPS.starhill = {
  ...
  enc: { rate: 16, list: [["ヨルネコ", 45, 50, 14], ["シャドネコ", 46, 50, 14], ["ムラサキビ", 47, 50, 12], ["ヤミノヌシ", 45, 50, 12], ["ホシミノ", 46, 50, 10], ["オバケシ", 47, 50, 8]] },
};
...
for (const [id, landmarks] of Object.entries(ROUTE_LANDMARKS)) {
  if (LEGACY_MAPS[id]) LEGACY_MAPS[id].landmarks = landmarks;
}

rebuildOutdoorWorld(LEGACY_MAPS);
applyFirstRegionV4(LEGACY_MAPS);
applySecondRegionV4(LEGACY_MAPS);
applyThirdRegionV4(LEGACY_MAPS);
applyFourthRegionV4(LEGACY_MAPS);
applyFifthRegionV4(LEGACY_MAPS);
applySixthRegionV4(LEGACY_MAPS);
applySeventhRegionV4(LEGACY_MAPS);
applyEighthRegionV4(LEGACY_MAPS);
