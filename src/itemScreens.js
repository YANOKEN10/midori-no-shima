import {salePrice} from './itemSelling.js';
...
export function drawItemList(w){
 ...
 if(selected){drawItem(G.ctx,selected.name,21,96,60);G.textCenter('もっている',51,158,3,10);G.textCenter((selected.n||0)+'こ',51,171,3,11);}
 if(!list.length)G.text('ありません',120,28,3,14);
 const top=Math.max(0,Math.min(w.i-5,list.length-6));
 list.slice(top,top+6).forEach((e,row)=>{const y=16+row*27,chosen=top+row===w.i;G.ctx.fillStyle=chosen?'#d7ede4':'#eaf3e9';G.ctx.fillRect(108,y-3,199,25);if(chosen)G.text('▶',108,y,3,11);G.textFit(e.name,123,y,shop||sell?125:139,3,13);const suffix=shop?item(e.name).price+'円':sell?salePrice(e.name)+'円':'×'+(e.n||1);G.textRight(suffix,303,y+1,3,11);});
 if(top>0)G.text('▲',293,9,3,9);if(top+6<list.length)G.text('▼',293,178,3,9);
 ...
 G.text('A えらぶ   B もどる'+(w.pockets?'   ← → ポケット':''),17,263,3,10);
}
