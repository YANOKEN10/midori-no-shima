export const grassMap151=id=>['route3','remoteLake','mountainAltar','resure','flowerPark','gaonPark','gaonParkWoods','gaonParkWetland','gaonParkFlowers','gaonParkHill'].includes(id);
export function upgradeParkGround151(base,d,getCatalog,source,floors){
 ...
 const objects=d.objects.map(o=>{const p=source(base,o,cat);if(['remoteLake','mountainAltar'].includes(base.id)&&o.id.startsWith('g:')&&p?.tile==='"')return {...o,template:'legacy73-grass-v25-mountain'};return !['route3','remoteLake','mountainAltar'].includes(base.id)&&p&&(p.group==='grass'||p.tile==='"')?{...o,stored79:true}:o;});
