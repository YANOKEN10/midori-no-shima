...
export function resourceNodes80(map){
 ...
 const added=new Set((map.editorAddedProps72||[]).map(p=>p.x+','+p.y+','+p.art));
 const candidates=props.filter(p=>(map.editor72?!!p.resource89||/^resource83-blue-(tree|rock)$/.test(p.art||''):!added.has(p.x+','+p.y+','+p.art)||/^resource83-blue-(tree|rock)$/.test(p.art||''))&&/rock|crag|tree|conifer|broadleaf/i.test(p.art||'')).map(p=>({...p,type:/rock|crag/i.test(p.art)?'rock':'tree'}));
