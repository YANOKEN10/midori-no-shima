import {target173} from './layout173.mjs';
import {maps160,applyLayout160,connectStation160} from './layout160.mjs';
import {grassMap151} from './parkGround151.mjs';
...
export async function loadPublishedMaps(maps){try{
 ...
 const [published,species]=await Promise.all([shared,fetch(new URL('../assets/editor-v72/species.json',import.meta.url),{signal:AbortSignal.timeout(7000)}).then(r=>r.json())]);extendMaps82(maps,published.definitions);const edits={...published.maps},cat=catalog(maps,species),base={...maps},moves=[],editCatalogs={};
 ...
 if(base.shop||base.hospital||Object.entries(edits).some(([id,d])=>base[id]&&validateEdit(base[id],d,cat).length)){
  ...
  for(const[id,d]of Object.entries(edits))if(base[id]&&validateEdit(base[id],d,cat).length&&editorBases[id]&&!validateEdit(editorBases[id],d,editorCat).length){base[id]=editorBases[id];maps[id]=editorBases[id];editCatalogs[id]=editorCat;}
 }
 for(const[id,m]of Object.entries(base))if((target173(m)||maps160(m.id)||m.fashionTown||m.id==='daycare'||grassMap151(m.id)||enclosedTown114(m)||m.id==='mountain'&&m.forestBorder||m.kind==='in'&&m.room?.rug)&&!edits[id])edits[id]=initial(m);
 for(const[id,edit]of Object.entries(edits)){const activeCat=editCatalogs[id]||cat;if(!base[id]||validateEdit(base[id],edit,activeCat).length)continue;maps[id]=applyEdit(base[id],edit,activeCat);for(let i=0;i<base[id].props.length;i++){const old=objectSource(base[id],{id:'p:'+i},cat),moved=maps[id].props.find(p=>p.editorObjectId72==='p:'+i);if(old.door&&moved?.door&&(moved.x!==old.x||moved.y!==old.y||moved.turn81))moves.push({map:id,x:(old.originalDoor81||old.door).x,y:(old.originalDoor81||old.door).y,nx:moved.door.x,ny:moved.door.y,turn:moved.turn81||0});}}
