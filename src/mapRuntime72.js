...
export async function loadPublishedMaps(maps){try{
 ...
 for(const[id,m]of Object.entries(base))if((maps160(m.id)||m.fashionTown||m.id==='daycare'||grassMap151(m.id)||enclosedTown114(m)||m.id==='mountain'&&m.forestBorder||m.kind==='in'&&m.room?.rug)&&!edits[id])edits[id]=initial(m);
