...
import {catalog,applyEdit,validateEdit} from './editorModel72.mjs';
let shared;
export async function loadPublishedMaps(maps){try{
 ...
 for(const[id,edit]of Object.entries(edits)){if(!base[id]||validateEdit(base[id],edit,cat).length)continue;maps[id]=applyEdit(base[id],edit,cat);for(let i=0;i<base[id].props.length;i++){const old=base[id].props[i],moved=maps[id].props.find(p=>p.editorObjectId72==='p:'+i);if(old.door&&moved?.door&&(moved.x!==old.x||moved.y!==old.y))moves.push({map:id,x:old.door.x,y:old.door.y,dx:moved.x-old.x,dy:moved.y-old.y});}}
