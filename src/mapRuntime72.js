...
export async function loadPublishedMaps(maps){try{
 ...
 for(const[id,m]of Object.entries(base))if((grassMap151(m.id)||enclosedTown114(m)||m.id==='mountain'&&m.forestBorder||m.kind==='in'&&m.room?.rug)&&!edits[id])edits[id]=initial(m);
 ...
 }catch(e){const cat=catalog(maps,[]);for(const[id,m]of Object.entries(maps))if((grassMap151(m.id)||enclosedTown114(m)||m.id==='mountain'&&m.forestBorder||m.kind==='in'&&m.room?.rug)&&!m.editor72)maps[id]=applyEdit(m,initial(m),cat);console.warn('公開マップを取得できなかったため標準マップを使います。');}addHomeSign117(maps.village);connectTwoCellExits140(maps);}
