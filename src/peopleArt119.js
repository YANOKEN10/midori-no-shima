...
function asset(key,battle=false){const id=(battle?'battle/':'map/')+key;if(!sheets.has(id)){const im=new Image();im.src=new URL('../assets/'+(battle&&/^npc-\d{2}$/.test(key)&&key!=='npc-02'?'trainers-v147/':/^hero-(?:boy-(?:fauxhawk|buzz)|girl-twintail)-(?:front|back)$/.test(key)?'hero-v127/':key==='rival-camo'||battle&&(key==='yanoken'||/^hero-(boy|girl)-.+-(front|back)$/.test(key))?'people-v121/':'people-v119/')+(battle&&/^npc-\d{2}$/.test(key)&&key!=='npc-02'?key:id)+'.png',import.meta.url).href;sheets.set(id,im);}return sheets.get(id);}
...
export function trainerBattle119(n){const key=n.name==='ヤノケン'||['v5:dex','voyage:yanoken','frontier:yanoken'].includes(n.script)?'yanoken':n.variant===2||n.look==='prof'||/スイス/.test(n.name||'')?'npc-02':battleKey119(Number.isInteger(n.variant)?n.variant:Number.isInteger(n.winterVariant)?34+n.winterVariant%4:({boy:0,girl:1,prof:2,oldman:3,nurse:4,clerk:5,hiker:6,sailor:12}[n.look]??0));if(!key)return null;const im=asset(key,true);return im.complete&&im.naturalWidth?im:null;}
