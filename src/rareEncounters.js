...
export function rareCandidates(mapId){
 ...
 const queue=[[m.spawn.x,m.spawn.y]],seen=new Set(),result=[],open=[];
 while(queue.length){const [x,y]=queue.pop(),key=x+','+y;if(seen.has(key)||!allowed.has(m.rows[y]?.[x]))continue;seen.add(key);
  if(!m.warps.some(w=>Math.abs(w.x-x)+Math.abs(w.y-y)<4)&&!m.npcs.some(n=>Math.abs(n.x-x)+Math.abs(n.y-y)<3)){const p={map:mapId,x,y};if(['"','C'].includes(m.rows[y][x]))result.push(p);else if([',','.'].includes(m.rows[y][x]))open.push(p);}
  queue.push([x-1,y],[x+1,y],[x,y-1],[x,y+1]);
 }
 return (result.length?result:open).sort((a,b)=>a.y-b.y||a.x-b.x);
}
export function createRareSpawns(seed=Math.floor(Math.random()*4294967296),existing={}){
 ...
 for(const rule of RARE_RULES){const options=rareCandidates(rule.map).filter(p=>!used.has(p.map+':'+p.x+':'+p.y));if(!options.length)continue;
  const old=existing?.[rule.name],prior=Array.isArray(old)?old:old?[old]:[],chosen=[];
  for(let i=0;i<(rule.spots||1);i++){const pool=options.filter(p=>!chosen.some(q=>q.x===p.x&&q.y===p.y));const valid=prior[i]&&pool.find(p=>p.map===prior[i].map&&p.x===prior[i].x&&p.y===prior[i].y);const picked=valid||pool[Math.floor(random()*pool.length)];if(!picked)break;chosen.push(picked);used.add(picked.map+':'+picked.x+':'+picked.y);}
