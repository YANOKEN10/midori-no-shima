...
const images=new Map(ITEM_NAMES.filter(name=>!['マリンエンブレム','カラット・エンブレム','れんらくせんチケット'].includes(name)).map((name,i)=>{const im=new Image();im.src=new URL('../assets/items-v13/'+String(i).padStart(2,'0')+'.png',import.meta.url).href;return [name,im];}));
...
images.set('れんらくせんチケット',images.get('たいかいパス'));
