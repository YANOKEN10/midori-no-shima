...
export class FollowerTrail {
 ...
 record(x,y,dir){const end=this.points.at(-1);if(!end){this.reset(x,y,dir);return;}if(Math.hypot(x-end.x,y-end.y)>2.1){this.reset(x,y,dir);return;}if(Math.hypot(x-end.x,y-end.y)<=.0001){if(this.pose)this.pose.moving=false;return;}this.points.push({x,y,dir});
 ...
 traversed+=d;}
 if(pose){pose.moving=!!this.pose&&Math.hypot(pose.x-this.pose.x,pose.y-this.pose.y)>.0001;this.pose=pose;}else if(this.pose)this.pose.moving=false;
