import {rivalName168,walkRival168} from './rival168.mjs';
...
export function refreshRival122(w){for(const n of w.npcs)if(n.rival122){n.displayName=rivalName168(State.save,n.displayName||'レイジ');const id=n.script.split(':')[1];n.gone=!available122(State.save,id)||(id==='marine'&&State.save.flags['rival122:marineExit']!==w.mapId);n.noRoam=true;}}
const canStand=(w,n,x,y)=>w.rivalCanStand122(x,y,n)&&Math.hypot(x-w.x,y-w.y)>.8;
const animate=(w,n,path)=>walkRival168(w,n,path,wait);
async function approach(w,n){const path=route122({x:Math.round(n.x),y:Math.round(n.y)},p=>Math.hypot(p.x-w.x,p.y-w.y)<=1.5,(x,y)=>canStand(w,n,x,y));if(path===null)return false;if(!await animate(w,n,path))return false;const dx=n.x-w.x,dy=n.y-w.y;w.dir=Math.abs(dx)>Math.abs(dy)?dx>0?'right':'left':dy>0?'down':'up';n.dir=({up:'down',down:'up',left:'right',right:'left'})[w.dir];return true;}
...
export async function runRivalEvent122(w,n,deps={}){const s=State.save,id=n.script.split(':')[1],event=RIVAL_EVENTS122[id],say=deps.say||((lines)=>ui.say(lines)),battle=deps.battle||startBattle,save=deps.persist||persist,leave=deps.depart||depart,now=deps.now||(()=>new Date());if(!event||!available122(s,id))return;
 const old=ui.speaker;ui.speaker=rivalName168(s,n.displayName||'レイジ');try{
 ...
 const result=await battle({trainer:{rivalBattle129:'regular',name:rivalName168(s,n.displayName||'レイジ'),appearance79:{...RIVAL_APPEARANCE122},fixedLevels122:true,party:event.team,money:0}});
