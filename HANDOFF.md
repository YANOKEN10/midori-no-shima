...
序盤11マップ実装済み。src/data/chapterOne.js、src/chapterStory.js、src/chapterArt.js が現行。正式仕様は docs/story-canonical.md、制作・検証・公開は docs/chapter-one-release.md。以下の旧記述より現行資料を優先する。

---

# ストーリーの訂正（2026-09-07）

ユーザー指定の正式仕様は docs/story-canonical.md。ラテット目撃後、スイスはかせからラグネット15個。ロッズタウンの少年ヤノケンから図鑑。母の贈り物と七つの谷の旧筋書きは置き換える。コードは未移行。

# 現行の作業場所（2026-09-06）

- 正式な継続作業フォルダ: `I:\2026\Claude code\gaon-world`
- 復元元: `I:\2026\Codex-transfer-2026-09-06\archive-00.tar` 内の `2026-08-29/rpg-voraz-monsters-i-claude-code/gaon-world-edit`（基準コミット `63c2de2`）。
- `I:\2026\Claude code\voraz-monsters` は旧版。そちらへ上書きしない。
- 最新方針: 作業データ・画像・成果物・テスト記録はIドライブのみ。C/Dドライブへコピー・復元しない。
- 今回はローカルテスト版。公開・デプロイ・クラウドDB・課金設定は変更していない。
- 起動: このフォルダの `Start-Game.ps1`。テストURL: `http://127.0.0.1:5179/`。
- 通常の「さいしょから」→家のお母さんに話すとラグネット15個を一度だけ受け取る。既存セーブにも専用の受取フラグを用い、従来の持ち物を減らさない。
- 谷守は図鑑を渡す。ネット8個の旧処理は撤去。
- 捕獲したガオンで戦い、戦闘中の「どうぐ」→「ラグネットをつかう」で追加捕獲できる。
- 会話・選択肢・戦闘情報枠の余白を拡大。スマホのスティックで選択肢を上下できる。
- 36生成マップの描画、母の一度きりの贈り物、保存、捕獲→攻撃→再捕獲、家から屋外への移動をPC/スマホ相当で確認。実機iOS/PWAと全地形の見た目・衝突一致は今回の検証対象外。
- 作業台帳: `docs/world-v4-ledger.md`。検証記録: `artifacts/20260906-verification.json`。

---

以下は旧引き継ぎ資料です。旧パス、「画像なし」、過去の次回候補などは現在と一致しません。上記と最新台帳を優先してください。
