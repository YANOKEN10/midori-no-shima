...
const trainers=await p.evaluate(async()=>{const {MAPS}=await import('/src/data/maps.js?v=20260909-follow-v14');return ['route2','natureforest'].flatMap(id=>MAPS[id].npcs.map((n,i)=>n.trainer?{id,i}:null).filter(Boolean))});assert.equal(trainers.length,9);
