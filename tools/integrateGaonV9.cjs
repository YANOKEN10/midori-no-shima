...
for(const row of rows)for(let view=0;view<2;view++){
  const dest=`assets/monsters/redesign-v9/${view?'back':'front'}/${path.basename(row.files[view])}`;
...
const files=Object.fromEntries([...source.matchAll(/\s+([^\s:]+): "\.\.\/\.\.\/(assets\/monsters\/(?:battle|redesign-v8\/front|redesign-v9\/front)\/[^\"]+)"/g)].map(m=>[m[1],m[2]]));
...
for(const row of rows)publicHtml=publicHtml.replaceAll('/'+row.files[0],'/assets/monsters/redesign-v9/front/'+path.basename(row.files[0]));
...
for(const file of versionFiles){const old=fs.readFileSync(file,'utf8'),next=old.replaceAll('20260908-gaon-v8','20260908-gaon-v9');if(old!==next)fs.writeFileSync(file,next);}
console.log('Integrated 25 redrawn species; rebuilt 153-entry evolution catalog');
