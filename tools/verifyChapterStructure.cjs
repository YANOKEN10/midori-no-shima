...
const report=await p.evaluate(async()=>{const {MAPS}=await import('/src/data/maps.js?v=20260908-training-v10');const {solid}=await import('/src/tiles.js');const {SPECIES}=await import('/src/data/species.js');const out=[];for(const [id,m] of Object.entries(MAPS)){
