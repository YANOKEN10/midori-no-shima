...
(async () => {
  ...
  try {
    ...
    for (const row of rows) {
      if (!fs.existsSync(`work/redesign-v11-source/${row.index}.png`)) continue;
      const pair = await page.evaluate(async index => {
        const image = new Image();
        image.src = `/work/redesign-v11-source/${index}.png?t=${Date.now()}`;
        await image.decode();
        const results = [];
        for (let view = 0; view < 2; view++) {
          ...
          if (opaqueMatte && [5,9,10,15,16].includes(index)) {
      ...
      pair.forEach((base64, view) => {
        const folder = `assets/monsters/redesign-v11/${view?'back':'front'}`;
