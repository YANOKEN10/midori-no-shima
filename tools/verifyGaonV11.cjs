...
(async()=>{
 const base=process.argv[2]||'http://127.0.0.1:5179', rows=JSON.parse(fs.readFileSync('docs/gaon-redesign-v11.json','utf8'));
 ...
 assert.equal(await p.locator('article').count(),153);assert.equal(await p.locator('article img[src*="redesign-v11"]').count(),29);
 const data=await p.evaluate(async rows=>{
  ...
  for(const row of rows){const name=row.newName||row.name;if(!S[name])throw Error('Missing renamed species '+name);}
  ...
  for(const name of Object.keys(a.BATTLE_ART_FILES))for(const back of [false,true]){
   ...
   if(src.includes('redesign-v11')){
    v11++;const c=document.createElement('canvas');c.width=c.height=80;const ctx=c.getContext('2d');ctx.drawImage(im,0,0);const d=ctx.getImageData(0,0,80,80).data;let opaque=0;
  ...
  return {count,v11,kokegoroLevel:m.lv,kokegoroEvolution:r.evolve,otherEvolution:cr.evolve};
 },rows);assert.equal(data.count,306);assert.equal(data.v11,58);
 await p.locator('#search').fill('コケゴロ');assert.equal(await p.locator('article:visible').count(),1);assert.equal(await p.locator('.family:visible').count(),0);await p.screenshot({path:'artifacts/v11-kokegoro.png'});
 await p.locator('#search').fill('コケトロッコ');assert.equal(await p.locator('article:visible').count(),1);
 if(base.includes('127.0.0.1')){
  await p.goto(base+'/?v4test=route1');await p.waitForFunction(()=>window.VM);await p.waitForTimeout(800);await p.evaluate(()=>{VM.State.save.party=[VM.makeMon('コケゴロ',30)];VM.State.save.battleTerrain='grass';VM.startBattle({wild:VM.makeMon('タキビィ',20)});});await p.waitForTimeout(500);await p.screenshot({path:'artifacts/v11-battle.png'});
 }
 assert.deepEqual(errors,[]);fs.writeFileSync('artifacts/v11-verification.json',JSON.stringify({base,...data,unchangedFiles:Object.keys(hashes).length,errors},null,2));console.log(JSON.stringify({base,...data,errors}));
 const preview=await b.newPage({viewport:{width:1300,height:1400}});await preview.goto(base+'/gaon-zukan/');await preview.setContent('<style>body{font-family:sans-serif;background:#e7efea}main{display:grid;grid-template-columns:repeat(5,1fr);gap:8px}article{background:white;text-align:center;padding:12px}img{width:110px;height:110px;image-rendering:pixelated}h3{margin:4px;font-size:16px}</style><main>'+rows.map(r=>'<article><h3>'+(r.newName||r.name)+'</h3>'+[0,1].map(v=>'<img src="'+base+'/assets/monsters/redesign-v11/'+(v?'back':'front')+'/'+path.basename(r.files[v])+'">').join('')+'</article>').join('')+'</main>');await preview.evaluate(()=>Promise.all([...document.images].map(i=>i.decode())));await preview.locator('main').screenshot({path:'artifacts/v11-all-29.png'});
