...
//
//   ・メールアドレスは にゅうりょく しなくてもよい
//   ・ログインは「なまえ」でも「メールアドレス」でもできる
// ============================================================
  ...
  try {
    ...
    if (action === "check") {
      ...
      if (!NAME_RE.test(rawName)) {
        res.status(200).json({ free: false, message: "なまえは 2〜16もじ、スペースと @ は つかえません。" });
        return;
      }
      const u = await L.readUser(L.normId(rawName));
      res.status(200).json({ free: !u, message: u ? "その なまえは つかわれています。" : "この なまえは つかえます。" });
      return;
    }
    ...
    if (action === "signup") {
      ...
      if (!NAME_RE.test(rawName)) {
        res.status(400).json({ error: "name", message: "なまえは 2〜16もじ、スペースと @ は つかえません。" });
        return;
      }
      ...
      if (await L.readUser(id)) {
        res.status(409).json({ error: "taken", message: "その なまえは もう つかわれています。" });
        return;
      }
    ...
    if (action === "login") {
      ...
      const pw = String(b.pw == null ? "" : b.pw);
      if (!who || !pw) {
        res.status(400).json({ error: "input", message: "なまえ（またはメール）と あいことばを いれてください。" });
        return;
      }
      const user = await L.findUser(who);
      if (!user || !L.checkPw(pw, user)) {
        await L.slowDown();
        res.status(401).json({ error: "auth", message: "なまえ か あいことばが ちがいます。" });
        return;
      }
