...
//  サーバー側の共通処理
//   ・アカウント1つ＝JSONファイル1つ
//   ・あいことば(パスワード)は scrypt でハッシュ化して保管
//   ・メールアドレスは「あってもなくてもよい」
//       - 無い人  … なまえ ＋ あいことば でログイン
//       - 有る人  … なまえ でも メール でもログインできる
// ============================================================
...
const ORIGINS = [
  "https://midori-no-shima.vercel.app",
  "https://yanoken10.github.io",
  "http://localhost:5179",
  "http://127.0.0.1:5179",
];
const PREVIEW = /^https:\/\/midori-no-shima-[a-z0-9-]+\.vercel\.app$/;
...
function notReady(res) {
  res.status(503).json({
    error: "setup",
    message: "クラウドの準備がまだです。「この たんまつだけ」で あそべます。",
  });
}
...
function userKey(id) {
  return "vm/u/" + crypto.createHash("sha256").update("vmon1:" + id).digest("hex") + ".json";
}
