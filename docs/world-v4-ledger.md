...
### 2026-09-06 生成マップの試遊版・母からのラグネット

- 作業場所を `I:\2026\Claude code\gaon-world` に復元。既存の生成画像36枚を使用し、元の画像を変更・削除していない。
- 主人公の家の初回会話で、母ミレナからラグネット15個を受け取る。再会話・再入場では増えず、専用フラグと個数を保存。
- 谷守の旧8個配布は廃止し、図鑑授与をネット入手フラグから独立させた。
- 捕獲した仲間での戦闘、戦闘中の「どうぐ」から「ラグネットをつかう」を確認。捕獲完了時に次の戦闘の説明を追加。
- 新規開始はV4の家の開始地点へ。古い研究所案内を母への案内に変更。
- 会話の左右24px・上20px、選択肢の上下16px・右24pxを確保。戦闘情報パネルも余白を拡大。
- スマホ用スティックをメニュー選択にも対応。
- PCとスマホ相当で、贈り物・二重受取防止・保存・谷守の図鑑・捕獲→技の使用→道具で再捕獲・玄関移動に合格。各環境で全36マップの実描画を確認し、ブラウザ例外・ローカルHTTPエラー0。
- 捕獲の回帰試験は乱数をテスト内で固定し、ゲーム本体の捕獲確率は変更していない。
- 実機iPhone/Android、インストール済みPWA、全画像と衝突マスクの視覚的一致は引き続き未検証。公開は未実施。
